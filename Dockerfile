FROM alpine:3.5
MAINTAINER SmartinfoLogiks <admin@smartinfologiks.com>
...
RUN cp /usr/bin/pecl /usr/bin/pecl_backup

COPY src/pecl /usr/bin/pecl
RUN chmod +x /usr/bin/pecl

#RUN pecl install mongodb

#RUN echo 'extension=mongodb.so' > /etc/php5/php.ini
